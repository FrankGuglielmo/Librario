...
@Observable class ArcadeGameManager2: GameManagerProtocol {
    ...
    init(dictionaryManager: DictionaryManager, userData: UserData? = nil) {
        ...
        let loadedGameState = GameState.loadGameState(gameMode: .arcade)
        self.gameState = loadedGameState ?? GameState()
        self.gameState.gameMode = .arcade // Ensure arcade mode
        ...
        let loadedTileManager = TileManager.loadTileManager(dictionaryManager: dictionaryManager, for: .arcade)
        
        if let tileManager = loadedTileManager {
            self.tileManager = tileManager
        } else {
            // Create a new TileManager for Arcade mode
    ...
    required init(from decoder: Decoder) throws {
        ...
        gameState.gameMode = .arcade // Ensure arcade mode
    ...
    func startNewGame(userStatistics: UserStatistics) {
        ...
        // Start the fire tile timer for arcade mode
    ...
    func saveGame() {
        ...
        sessionData.saveSessionData()
        tileManager.saveTileManager(for: .arcade)
    }
    ...
    private func handleGameplayStateChange(from oldState: GameManager.GameplayState, to newState: GameManager.GameplayState) {
        ...
        // Handle Arcade-specific state transitions
        switch newState {
        case .active:
